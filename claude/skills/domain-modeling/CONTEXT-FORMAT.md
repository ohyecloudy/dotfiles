...
- **의견을 분명히 할 것.** 같은 개념에 여러 단어가 있으면 최선을 하나 고르고 나머지는 `/Avoid/`에 나열.
...
- **자연스러운 묶음이 생기면 소제목으로 그룹화.** `* Language` 아래 `** {그룹}` → `*** {용어}`. 모든 용어가 하나의 응집된 영역에 속하면 평면 목록(`** {용어}`)도 무방.
...
아래 경로는 in-repo 기준. 외부(`~`) 모드면 같은 트리가 `~/domain-models/<정규화 경로>/` 아래에 미러된다(위치 판정은 [SKILL.md](./SKILL.md)의 "저장 위치 판정" 참고).

**단일 컨텍스트(대부분의 저장소):** 저장소 루트에 `CONTEXT.org` 하나.

**다중 컨텍스트:** 저장소 루트의 `CONTEXT-MAP.org`가 컨텍스트 목록, 위치, 상호 관계를 나열:
...
- [[file:./src/ordering/CONTEXT.org][Ordering]] - 고객 주문을 접수하고 추적
- [[file:./src/billing/CONTEXT.org][Billing]] - 인보이스를 생성하고 결제를 처리
- [[file:./src/fulfillment/CONTEXT.org][Fulfillment]] - 창고 피킹과 배송을 관리
...
- `CONTEXT-MAP.org`가 있으면 읽어서 컨텍스트를 찾음
- 루트에 `CONTEXT.org`만 있으면 단일 컨텍스트
- 둘 다 없으면 첫 번째 용어가 확정될 때 루트 `CONTEXT.org`를 생성
